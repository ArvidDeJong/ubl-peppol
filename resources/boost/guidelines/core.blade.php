...
- The VAT category (`tax_category_id`) is a tax decision; `Darvis\UblPeppol\Vat\VatCategory` explains each one (`description()`, `rules()`, `VatCategory::guide('nl')`). `K` is an intra-community supply, `AE` reverse charge, `E` exempt, `G` export outside the EU, `O` not subject to VAT; all at 0%. Their VAT breakdown needs an exemption reason: pass `tax_exemption_reason_code` (BT-121, `VatExemptionReason`) and/or `tax_exemption_reason` (BT-120, any language) in the `addTaxTotal()` entry. For `K`, `AE`, `G` and `O` the builder writes the matching `VATEX-EU-...` code when you pass none; for `E` you must pass one. A reason on `S` or `Z` throws. Category `K` also needs `addDelivery()` with the date and the country (BR-IC-11, BR-IC-12).
